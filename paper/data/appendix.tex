...
\section{Layout estimation}
Estimating the layout of a 3D scene, including the corners, the edges and the planes, is an important and newly emerged task in scene reconstruction. Based on Mask R-CNN\cite{he2017mask}, PlaneRCNN\cite{liu2019planercnn} can reconstruct the planar regions of a scene giving a single RGB image of the scene. LCNN\cite{zhou2020endtoend} enables us to obtain a vectorized wireframe of the scene. SceneCAD\cite{avetisyan2020scenecad}, however, introduces a novel method to estimate the corners, the edges and the planes of the scene in a single procedure. 
...
\cite{zhang2020geolayout}
\cite{liu2019planercnn}
\cite{zhou2020endtoend}
\cite{DBLP:journals/corr/LinCDWQH17}
